...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  itemHtml = itemHtml.replace(/{{checkQuote}}/g, buildCheckQuoteLink(data.id));
  ...
  itemHtml = itemHtml.replace(/{{itemSKU}}/g, buildSkus(data));
  return itemHtml;
};
...
function buildCheckQuoteLink(id){
  var stringhtml = '';
          
  if(getCookie('is_login')== 1 && getCookie('is_admin')==1){
    stringhtml ='<div class="latest_quote_wrapper"><div class="latest_quote_data" id="'+id+'">No Quote</div></div>'; 
    $.ajax({
       type:'GET',
       url:webhook_domain+"/shopify/get-latest-quote/"+id,
       data: '',
       success:function(data){
           ...
           $('#details-'+id).click(function() {
             var prod_id = $(this).attr('data-id');
              $.ajax({
                 type:'GET',
                 url:webhook_domain+"/shopify/collection-quote-history-details/"+prod_id,
                 data: '',
                 success:function(data1){
                      var history = JSON.parse(data1);
                      ...
                      for (var key in history) {
                        ...
                        var stretch_price = Math.round(history[key].product_stretch_price);
                        ...
                        data_val += '<td>'+stretch_price.formatMoney(2)+'</td>';
                      ...
                      var historyDialog = new Fancybox([{
                        src: '<table class="table table-bordered latest-quote-table">'+
                              '<thead>'+
                                '<tr>'+
